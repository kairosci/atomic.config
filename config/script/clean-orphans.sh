...
check-package-exists() {
    ...
    if flatpak list --app --columns=application | grep -qi "$name"; then
        return 0
    fi
    
    if rpm -qa | grep -qi "$name"; then
        return 0
    fi
    
    if command -v "$name" &>/dev/null || command -v "$lower_name" &>/dev/null; then
        return 0
    fi
